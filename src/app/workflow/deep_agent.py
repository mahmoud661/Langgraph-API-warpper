...
from src.app.llm_provider import get_llm
from src.app.workflow.backends.protocol import BackendProtocol
...
def get_default_model() -> BaseChatModel:
    """Get the default model from llm_provider based on environment configuration."""
    return get_llm()
...
def create_deep_agent(
    ...
    *,
    ...
    if hasattr(model, "profile") and isinstance(getattr(model, "profile", None), dict):
        profile = model.profile
        if "max_input_tokens" in profile and isinstance(
            profile["max_input_tokens"], int
        ):
...
def make_graph(config=None):
    """Wrapper function for LangGraph that takes RunnableConfig."""
